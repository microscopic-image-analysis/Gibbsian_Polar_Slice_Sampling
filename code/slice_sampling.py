"""
Provides implementations of several efficient slice sampling algorithms
...
def orbit_shrinkage(log_trafo, r_old, p, log_t):
    """Auxiliary function, not to be called by the user"""

    y = ssf.uniform_great_subsphere(p)
...
def gibbsian_polar_ss(log_density, x_0, w, itnum, bar=True):
    ...
    for n in nrange(itnum, bar):
        ...
        Theta[n] = orbit_shrinkage(log_trafo, R[n-1], Theta[n-1], log_t)
        R[n] = radius_shrinkage(log_trafo, w, R[n-1], log_t, Theta[n])
    return R.reshape(-1,1) * Theta
...
def naive_elliptical_ss(log_density, x_0, itnum, bar=True):
    """Runs the Naive Elliptical Slice Sampler, using the multivariate standard
        ...
        Returns:
            samples: np array of shape (itnum+1, d), where samples[i] is the 
                i-th sample generated by the naive ESS
    """

    log_likelihood = lambda x: -log_prior_identity(1.0, x) + log_density(x)
    d = x_0.shape[0]
    X = np.zeros((itnum+1, d))
    X[0] = x_0
    for n in nrange(itnum, bar):
...
def elliptical_ss(cov, log_likelihood, x_0, itnum, bar=True):
    ...
    if type(cov) == float:
        ...
        chol = alg.cholesky(cov)
        sample_y = lambda: chol @ rnd.normal(0.0, 1.0, d)
    X = np.zeros((itnum+1, d))
    X[0] = x_0
    for n in nrange(itnum, bar):
        log_t = log_likelihood(X[n-1]) + np.log(rnd.uniform())
